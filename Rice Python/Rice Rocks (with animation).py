...
def group_collide(group, other_object):
    ...
    for each in group:
        if (each.collide(other_object)):
            ...
            explosion = Sprite(each.get_position(),[0,0],0,0,explosion_image,explosion_info,explosion_sound)
...
class Sprite:
    ...
    def draw(self, canvas):
        if (not self.animated):
            ...
            canvas.draw_image(self.image,[self.image_center[0]+(128*self.age),self.image_center[0]],self.image_size,self.pos,self.image_size,self.angle)
...
def draw(canvas):
    ...
    #process rocks and missiles and explosions
