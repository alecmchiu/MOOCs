...
def updateHand(hand, word):
    ...
    for letter in set_word:
        new_hand[letter] = hand.get(letter, 0) - word.count(letter)
    return new_hand
...
def isValidWord(word, hand, wordList):
    ...
    if lower_word not in wordList:
        return False
    for element in set(lower_word):
        if lower_word.count(element) > hand.get(element,0):
            return False
    return True
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    original_hand_length = calculateHandlen(hand)
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                word_score = getWordScore(user_word, original_hand_length)
                score += word_score  
                print(" \" {} \" earned {} points. Total: {} points\n".format(user_word, word_score, score))  
                # Update the hand 
